...
const getDashboardStats = async (days = 30) => {
  ...
  const viewsChart =
    Object.entries(byDay).map(
      ([date, views]) => ({
        date,
        views,
        unique:
          uniqueByDay[date]?.size ||
          0
      })
    );
  ...
  const { data: topArticles } =
    await supabase
      .from('articles')
      .select(
        'id, title, slug, views, cover_image_url, collaborators(name)'
      )
      .eq(
        'status',
        'published'
  ...
  const { data: topShares } =
    await supabase
      .from('article_shares')
      .select(
        'article_id, platform, articles(title, slug)'
      )
      .gte(
        'created_at',
        sinceISO
  ...
  const { data: topLikes } =
    await supabase
      .from('article_likes')
      .select(
        'article_id, articles(title, slug, cover_image_url)'
      )
      .gte(
        'created_at',
        sinceISO
